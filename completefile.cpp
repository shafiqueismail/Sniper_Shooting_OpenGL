// ===== MERGED FILE: gunsUpdate.cpp + stars.cpp =====

// ===== START gunsUpdate.cpp =====
...
// ======================= AUDIO (miniaudio) =======================
// Download miniaudio.h from https://miniaud.io and put it beside this .cpp
...
// ===== START stars.cpp =====
    ...
    void setupBuffers() {
        ...
        if (!texCoords.empty()) {
            ...
            // If no texture coordinates, create default ones
            std::vector<glm::vec2> defaultTexCoords(vertices.size(), glm::vec2(0.5f, 0.5f));
            glBindBuffer(GL_ARRAY_BUFFER, vbo[2]);
            glBufferData(GL_ARRAY_BUFFER, defaultTexCoords.size() * sizeof(glm::vec2), defaultTexCoords.data(), GL_STATIC_DRAW);
            glEnableVertexAttribArray(2);
            glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, nullptr);
        }
...
void renderAsteroids(const mat4& view, const mat4& projection, GLuint shader) {
    ...
    void loadSatelliteModels() {
        ...
        quikscat.rotationSpeed = 0.075f;   // rotation speed

        temp_uvs.clear();

        // add to satellite vector
        satellites.push_back(tess);
        satellites.push_back(quikscat);
    }

    // render satellites to screen
    void renderSatellites(const mat4 &view, const mat4 &projection, GLuint satelliteShader) {
        // shader
        glUseProgram(satelliteShader);

        // get earth's position (satellites orbit earth!)
        vec3 earthPos;
        int planetIndex = 0;
        for (const auto &planet : planets)
        {
        ...
        // sun positions and colors (for lighting)
        vec3 sunPositions[2];
        vec3 sunColors[2];
        planetIndex = 0;
        int sunCount = 0;
    ...
    // adjusts satellites position every frame
    void updateSatellites(float deltaTime, const vec3& earthPosition) {
        // for both satellites
        for (auto& sat : satellites) {
